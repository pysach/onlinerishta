...
<div class="grid_3">
  ...
  <div class="col-sm-2"><h1><a href="vanc.php?reset=$4%$#&&%">All profile</a></h1></div>
  <div class="col-sm-2"><h1><a href="members.php">Premium Members</a></h1></div>     
          <form action="vanc.php"  method="POST">
...
$stmt = $con->prepare("SELECT COUNT(tbl) as idd FROM sd_client where sd_sex=:gen and scast=:scast $j"); 
$stmt->execute(array(':gen'=>$g,':scast'=>$scastt));
...
?>
 ...
 $stmt=$con->prepare("SELECT * FROM sd_client where sd_sex IN(:gen) and scast=:scastt $j ORDER BY rand() LIMIT $start_from, $limit"); 
 $stmt->execute(array(':gen'=>$g,':scastt'=>$scastt));
...
for ($i=1; $i<=$total_pages; $i++) {  
             $pagLink .= "<li><a href='vanc.php?page=".$i."'>".$i."</a></li>";  
};  
echo $pagLink . "</ul></nav>";  
?>
