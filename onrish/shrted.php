...
<!--        -->
  ...
  <div class="col-md-4"><h1>Shortlited Profile </h1></div>
     
          <form action="shrted.php"  method="POST">
...
$stmt = $con->prepare("SELECT COUNT(id) as idd FROM shortlist where tap=:use $j "); 
$stmt->execute(array(':use'=>$user));
$row = $stmt->fetch(PDO::FETCH_ASSOC);
$total_records = $row['idd'];
$total_pages = ceil($total_records / $limit);  
$pagLink = "<nav><ul class='pagination'>";
for($i=1; $i<=$total_pages; $i++) {  $pagLink .= "<li><a href='shrted.php?page=".$i."'>".$i."</a></li>";   };
echo $pagLink . "</ul></nav>";  
?>
...
$stmt=$con->prepare("SELECT * from sd_client INNER JOIN shortlist on sd_client.tbl=shortlist.sid and shortlist.tap=:uss $j ORDER BY tbl desc  LIMIT $start_from, $limit"); 
$stmt->execute(array(':uss'=>$user));
...
<?php
$stmt2=$con->prepare("SELECT * FROM sdetails where did=:sdid order by did desc "); 
$stmt2->execute(array(':sdid'=>$x));
...
?>				    
                    <tr class="opened">
						...
						<td class="day_value"><?php echo $row2['sheight']; ?></td>
					</tr>

                    <tr class="opened">
						<td class="day_label1">Profession :</td>
						<td class="day_value"><?php echo $row2['soccupation']; ?></td>
					</tr>
					
					<tr class="opened">
						<td class="day_label1">Cast:</td>
						<td class="day_value"><?php echo $row2['scast']; ?></td>
					</tr>
					
					<tr class="opened">
						<td class="day_label1">color Complextion:</td>
						<td class="day_value"><?php echo $row2['scolour']; ?></td>
					</tr>
					<tr class="opened">
						<td class="day_label1">Marital Status:</td>
						<td class="day_value"><?php echo $row2['sstatus'] ?></td>
					</tr>
					<tr class="closed">
						<td class="day_label1">Manglic Dosh:</td>
						<td class="day_value closed"><span><?php echo $row2['smangal']; ?></span></td>
					</tr>
...
<!--			   <a href="wait.php?id=<?php echo $row['tbl']; ?>"><div class="vertical">View Profile</div></a>-->
     			   <a href="wait.php?del=<?php echo $row['tbl']; ?>"><div class="vertical">Remove from Shortlist</div></a>
	               <a href="wait.php?ch_mail=<?php echo $row['tbl']?>&pg=<?php echo $page ?>"><div class="vertical">Send Interest</div></a>
		   </div>
	   <hr><hr>
	    </div>
...
for ($i=1; $i<=$total_pages; $i++) {  
             $pagLink .= "<li><a href='shrted.php?page=".$i."'>".$i."</a></li>";  
};  
echo $pagLink . "</ul></nav>";  
?>
...
$('.pagination').pagination({
		...
		currentPage : <?php echo $page;?>,
 		hrefTextPrefix : 'shrted.php?page='
    });
	});
